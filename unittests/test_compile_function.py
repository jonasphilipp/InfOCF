...
class TestCompileFunction(unittest.TestCase):
    ...
    def test_compile_function_with_custom_preocf(self):
        ...
        signature = ["a", "b"]
        custom_ranks = {"00": 0, "01": 1, "10": 2, "11": 0}
        ...
        a = Symbol("a", BOOL)
        b = Symbol("b", BOOL)
        rev_conds = [Conditional(a, b, "(a|b)"), Conditional(Not(a), b, "(!a|b)")]
        ...
        for i, entry in enumerate(result):
            ...
            for world in worlds_covered:
                satisfies_verification = (
                    custom_preocf.world_satisfies_conditionalization(
                        world, rev_cond.make_A_then_B()
                    )
                )
                satisfies_falsification = (

...
class InferenceCorrectnessTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.inference_systems = ['p-entailment', 'system-z', 'system-w', 'lex_inf', 'c-inference']
    ...
    def test_inference_correctness(self):
        for test_set in self.test_sets:
            ...
            self.assertTrue(os.path.isfile(test_set_path),
            ...
            for index, row in df.iterrows():
                with self.subTest(row=index):
                    ...
                    if (row['belief_base_filepath'] != belief_base_filepath or
                        row['queries_filepath'] != queries_filepath):
                        
                        collected_results = pd.DataFrame()
                        belief_base_filepath = str(row['belief_base_filepath'])
                        queries_filepath = str(row['queries_filepath'])
                        belief_base_filepath_full = os.path.join(self.BASE_DIR, belief_base_filepath)
                        queries_filepath_full = os.path.join(self.BASE_DIR, queries_filepath)
                        
                        self.assertTrue(os.path.isfile(belief_base_filepath_full),
                                        f"{belief_base_filepath} is not a valid file")
                        self.assertTrue(os.path.isfile(queries_filepath_full),
                                        f"{queries_filepath} is not a valid file")
                        
                        # Skip files based on defined conditions
                        if any(cond in belief_base_filepath for cond in self.SKIP_CONDITIONS):
                            continue
                    ...
                    self.assertFalse(current_result.empty,
                    ...
                    for inference_system in self.inference_systems:
                        ...
                        self.assertFalse(system_results.empty,
                        ...
                        if inference_system == row['inference_system']:
                            ...
                            self.assertEqual(index, last_index + 1,
                            ...
                            if inference_system in self.excluded_systems:
                                ...
                                self.assertEqual(
                                    system_results.iloc[0]['result'], row['result'],

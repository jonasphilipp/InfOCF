...
This module provides the main interface for performing inference operations on
conditional belief bases. It serves as the primary entry point for users who want
to perform inference without dealing with low-level implementation details.

The module includes:
- InferenceManager: Main class for inference operations
- create_epistemic_state: Function to initialize epistemic state
- create_inference_instance: Factory function for inference implementations

Main Features:
- Unified interface across all inference systems
- Automatic solver selection and optimization
- Comprehensive timeout and error handling
- Detailed performance monitoring and logging
- Support for parallel inference execution
"""
...
This function initializes the core data structure that holds all information about
a belief base and its associated inference parameters. The epistemic state serves
as the central hub for storing belief base data, solver configurations, and
intermediate results throughout the inference process.

Parameters
----------
belief_base : BeliefBase
    Parsed belief base containing conditionals and signature.
inference_system : str
    Name of the inference system to use. Must be one of:
    {"p-entailment", "system-z", "system-w", "c-inference", "lex_inf"}.
smt_solver : str
    SMT solver backend name (e.g., "z3").
pmaxsat_solver : str
    MaxSAT solver backend name (ignored for p-entailment and system-z).
weakly : bool
    Whether weakly consistent belief bases are accepted/handled.

Returns
-------
dict
    Initialized epistemic state dictionary containing:
    - belief_base: The parsed belief base
    - inference_system: The selected inference system
    - smt_solver: The SMT solver configuration
    - pmaxsat_solver: The MaxSAT solver configuration
    - preprocessing_done: Whether preprocessing has been completed
    - preprocessing_timed_out: Whether preprocessing timed out
    - preprocessing_time: Time spent on preprocessing (ms)
    - weakly: Whether weakly consistent belief bases are accepted/handled

Notes
-----
This function is typically called automatically by InferenceManager and should
not be called directly unless implementing custom inference workflows.
"""
...
"""Create and return the appropriate inference implementation instance.

This factory function instantiates the concrete inference class based on the
inference system specified in the epistemic state. It handles the selection
of the appropriate implementation (e.g., SystemZ, SystemW, LexInf, etc.) and
configures it with the provided epistemic state.

Parameters
----------
epistemic_state : dict
    Epistemic state dictionary created by create_epistemic_state().

Returns
-------
Inference
    Instance of the appropriate inference implementation class.

Raises
------
Exception
    If an invalid or unsupported inference system is specified.


Supported inference systems:
- "p-entailment"
- "system-z"
- "system-w"
- "c-inference"
...
class InferenceManager:
    """High-level inference manager for conditional belief bases.

    InferenceManager provides a unified interface for performing inference operations
    on conditional belief bases using various inference systems. It automatically
    handles the initialization of epistemic state, selection of appropriate inference
    implementations, and execution of inference queries.

    This class serves as the main entry point for users who want to perform inference
    operations without dealing with the low-level details of different inference systems
    and solver configurations.

    Attributes
    ----------
    epistemic_state : dict
        Internal dictionary containing all information about the belief base and
        inference configuration. This includes solver settings, preprocessing status,
        and intermediate results.

    Examples
    --------
    Basic usage with system Z:

    >>> from parser.Wrappers import parse_belief_base, parse_queries
    >>> from inference.inference_manager import InferenceManager
    >>>
    >>> bb = parse_belief_base("signature\\na,b\\n\\nconditionals\\nkb{(a|b)}")
    >>> queries = parse_queries("(b|a)")
    >>> manager = InferenceManager(bb, "system-z")
    >>> results = manager.inference(queries)
    >>> print(results)

    Using different inference systems:

    >>> # System W with Z3 optimization
    >>> manager = InferenceManager(bb, "system-w", smt_solver="z3", pmaxsat_solver="z3")
    >>> results = manager.inference(queries, total_timeout=30)
    """
    ...
    def __init__(
        ...
        """Initialize the InferenceManager with a belief base and inference configuration.

        This constructor sets up the inference environment by validating parameters,
        creating the epistemic state, and preparing for inference operations.

        Parameters
        ----------
        belief_base : BeliefBase
            Parsed belief base containing the conditionals and signature to reason about.
        inference_system : str
            The inference system to use. Must be one of:
            {"p-entailment", "system-z", "system-w", "c-inference", "lex_inf"}.
        smt_solver : str, default "z3"
            SMT solver backend to use for logical operations. Common options:
            "z3" (Z3 Theorem Prover), "cvc5", "yices".
        pmaxsat_solver : str, default "rc2"
            Partial MaxSAT solver for systems that require it. Options include:
            "rc2", "z3" (for Z3-based implementations).
            Ignored for p-entailment and system-z.
        ...
        ------
        AssertionError
            If the specified smt_solver is not available in the PySMT environment.
        ...
        >>>
        >>> bb = parse_belief_base("signature\\na,b\\n\\nconditionals\\nkb{(a|b)}")
        >>> manager = InferenceManager(bb, "system-z", smt_solver="z3")
        """
    ...
    """Execute inference queries against the belief base.

    This is the main method for performing inference operations. It takes a set of
    conditional queries and determines whether they can be inferred from the belief
    base under the configured inference system.

    The method handles the complete inference pipeline:
    1. Belief base preprocessing (if not already done)
    2. Query evaluation using the selected inference system
    3. Result collection and formatting
    4. Performance timing and timeout handling

    Parameters
    ----------
    queries : Queries
        Container of conditional queries to evaluate.
    total_timeout : int, default 0
        Global timeout budget in seconds for the entire operation.
        When set, preprocessing and inference timeouts are derived from this.
        0 disables global timeout.
    inference_timeout : int, default 0
        Per-query timeout in seconds. 0 disables per-query timeout.
    preprocessing_timeout : int, default 0
        Timeout for belief base preprocessing in seconds.
    ...
    decimals : int, default 1
        Number of decimal places to round timing measurements to.

    Returns
    -------
    pandas.DataFrame
        Results dataframe with the following columns:
        - index: Query index in the original queries object
        - result: Boolean inference result (True if query is entailed)
        - signature_size: Number of atoms in the belief base signature
        - number_conditionals: Number of conditionals in the belief base
        - preprocessing_time: Time spent on preprocessing (seconds)
        - inference_time: Time spent on inference for this query (seconds)
        - preprocessing_timed_out: Whether preprocessing timed out
        - inference_timed_out: Whether this query timed out
        - belief_base: Name/label of the belief base
        - queries: Name/label of the queries batch
        - query: String representation of the query
        - inference_system: Name of the inference system used
        - smt_solver: SMT solver backend used
        - pmaxsat_solver: MaxSAT solver backend used (if applicable)
    ...
    >>> manager = InferenceManager(bb, "system-z")
    >>> results = manager.inference(queries)
    >>> print(results[['query', 'result', 'inference_time']])

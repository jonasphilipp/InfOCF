...
class LexInfZ3(Inference):
    ...
    def _rec_inference(
        self, opt_v: Optimize, opt_f: Optimize, partition_index: int, query: Conditional
    ) -> bool:
        ...
        for xi_i in [s for s in xi_i_set if len(s) == v]:
            for xi_i_prime in [s for s in xi_i_prime_set if len(s) == f]:
                ...
                [
                    opt_v.add(c.make_A_then_not_B() == False)
                    for c in part
                    if c not in xi_i
                ]
                [opt_f.add(c.make_A_then_not_B()) for c in xi_i_prime]
                [
                    opt_f.add(c.make_A_then_not_B() == False)
                    for c in part
                    if c not in xi_i_prime
                ]

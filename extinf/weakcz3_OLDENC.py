...
class WeakCz3OLD():
    ...
    def compile_and_encode_query(self, query):
        ...
        ands.append(ors)
        ands2 = [(v <= i) for i in vSum[0]]
        ors2 = z3.Not(z3.And([(v<i) for i in vSum[0]]))
        ands2.append(ors2)
        ands.extend(ands2)
        ands.append(v>=f)
        return ands
    ...
    def minima_encoding(self, mv: int, eta:int, ssums: list) -> list:
    ...
    def encoding(self, etas: dict, vSums: dict, fSums: dict) -> list:
        ...
        for index, eta in etas.items():
            ...
            vMin = self.minima_encoding(mv, eta, vSums[index])
            fMin = self.minima_encoding(mf, eta, fSums[index])
            csp.extend(fMin)
            csp.extend(vMin)

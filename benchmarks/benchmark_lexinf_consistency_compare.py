...
def completed_keys(output_path: Path) -> set[tuple[str, str]]:
    if not output_path.exists() or output_path.stat().st_size == 0:
        return set()
    df = pd.read_csv(output_path, usecols=["dataset", "belief_base"])
    return set(
        zip(df["dataset"].astype(str), df["belief_base"].astype(str), strict=False)
    )

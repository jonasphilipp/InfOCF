...
from pysmt.shortcuts import TRUE, Not, Symbol
...
# ---------------------------
def _make_belief_base(
    n_vars: int, n_conds: int, seed: int, make_unsat: bool
) -> BeliefBase:
    """
    ...
    if make_unsat:
        p0 = vars_[0]
        c_pos = Conditional(
            consequence=p0, antecedence=TRUE(), textRepresentation="(p0|⊤)"
        )
        c_pos.index = idx
        conds[idx] = c_pos
        idx += 1
        ...
        c_neg = Conditional(
            consequence=Not(p0), antecedence=TRUE(), textRepresentation="(¬p0|⊤)"
        )
        c_neg.index = idx
        conds[idx] = c_neg
        idx += 1
...
def bbs():
    ...
    return {
        ...
        "sat_medium": _make_belief_base(
            n_vars=25, n_conds=35, seed=2, make_unsat=False
        ),
        "unsat_small": _make_belief_base(
            n_vars=12, n_conds=15, seed=3, make_unsat=True
        ),
        "unsat_medium": _make_belief_base(
            n_vars=25, n_conds=35, seed=4, make_unsat=True
        ),
    }
...
@pytest.mark.parametrize(
    "case", ["sat_small", "sat_medium", "unsat_small", "unsat_medium"]
)
